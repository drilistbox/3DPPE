...
info_path = os.path.join(data_root, 'petr_sanchu', 'mmdet3d_nuscenes_30f_infos_{}.pkl'.format(info_prefix))
...
def add_frame(sample_data, e2g_t, l2e_t, l2e_r, e2g_r):
    sweep_cam = dict()
    ...
    sweep_cam['ego2global_translation'] = pose_record['translation']    # ego --> global
    sweep_cam['ego2global_rotation'] = pose_record['rotation']
    sweep_cam['sensor2ego_translation'] = calibrated_sensor_record['translation']   # sensor --> ego
    sweep_cam['sensor2ego_rotation'] = calibrated_sensor_record['rotation']
    sweep_cam['cam_intrinsic'] = calibrated_sensor_record['camera_intrinsic']       # intrinsic
    ...
    # reference lidar --> ego
    car_from_lidar = transform_matrix(translation=l2e_t, rotation=Quaternion(l2e_r), inverse=False)
    # ego --> global
    global_from_car = transform_matrix(translation=e2g_t, rotation=Quaternion(e2g_r), inverse=False)
    # global --> sweep ego
    sweep_car_from_global = transform_matrix(translation=e2g_t_s, rotation=Quaternion(e2g_r_s), inverse=True)
    # sweep ego --> sweep camera
    sweep_cam_from_car = transform_matrix(translation=l2e_t_s, rotation=Quaternion(l2e_r_s), inverse=True)

    # reference lidar --> sweep_cam
    lidar2cam_rt = reduce(np.dot, [sweep_cam_from_car,  sweep_car_from_global, global_from_car, car_from_lidar])
...
for current_id in tqdm.tqdm(range(len(key_infos['infos']))):
    ...
    print('l2e_t{}-l2e_r{}-e2g_t{}-e2g_r{}'.format(['{:0.6f}'.format(tem) for tem in l2e_t], ['{:0.6f}'.format(tem) for tem in l2e_r], ['{:0.6f}'.format(tem) for tem in e2g_t], ['{:0.6f}'.format(tem) for tem in e2g_r]))
    ...
    for i in range(num_prev):  #### previous sweep frame  
        ...
        for j in range(num_sweep): 
            ...
            for cam in sensors: 
                ...
                sweep_cam = add_frame(sample_data, e2g_t, l2e_t, l2e_r, e2g_r)
