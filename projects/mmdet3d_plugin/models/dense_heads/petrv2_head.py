...
class SELayer(nn.Module):
    ...
    def forward(self, x, x_se):
        ...
        Args:
            ...
            x_se: image feature     (B*N, embed_dims, H, W)
        Returns:
...
class PETRv2Head(AnchorFreeHead):
    ...
    def position_embeding(self, img_feats, img_metas, masks=None):
        ...
        img2lidars = coords.new_tensor(img2lidars)      # (B, N, 4, 4)  N=N_view * N_frame

        # (1, 1, W, H, D, 4, 1) --> (B, N, W, H, D, 4, 1)
        coords = coords.view(1, 1, W, H, D, 4, 1).repeat(B, N, 1, 1, 1, 1, 1)
        # (B, N, 1, 1, 1, 4, 4) --> (B, N, W, H, D, 4, 4)
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        Args:
            mlvl_feats (tuple[Tensor]): Features from the upstream
                ...
                (B, N, C, H, W).   N = N_view * N_frame    # List[(B, N, C'=256, H'/16, W'/16), (B, N, C'=256, H'/32, W'/32), ]
        Returns:
        ...
        x = mlvl_feats[self.position_level]     # (B, N, C=256, H, W)  只选择一个level的图像特征.
        batch_size, num_cams = x.size(0), x.size(1)

        input_img_h, input_img_w, _ = img_metas[0]['pad_shape'][0]
        # 建立masks，图像中pad的部分为1, 用于在attention过程中消除pad部分的影响.
        masks = x.new_ones(
            (batch_size, num_cams, input_img_h, input_img_w))   # (B, N, img_H, img_W)
        for img_id in range(batch_size):
            for cam_id in range(num_cams):
                img_h, img_w, _ = img_metas[img_id]['img_shape'][cam_id]
                masks[img_id, cam_id, :img_h, :img_w] = 0

        # (B*N, C, H, W) --> (B*N, C'=embed_dim, H, W)
        x = self.input_proj(x.flatten(0, 1))
        x = x.view(batch_size, num_cams, *x.shape[-3:])     # (B, N, C'=embed_dim, H, W)

        # interpolate masks to have the same spatial shape with x
        masks = F.interpolate(
            masks, size=x.shape[-2:]).to(torch.bool)     # (B, N, H, W)
        ...
        if self.with_position:
            ...
            if self.with_fpe:
                coords_position_embeding = self.fpe(
                    coords_position_embeding.flatten(0, 1), x.flatten(0, 1)).view(x.size())   # (B, N, embed_dims, H, W)

            pos_embed = coords_position_embeding

            if self.with_multiview:
                # 加入 2D PE 和 multi-view prior
                sin_embed = self.positional_encoding(masks)     # (B, N, num_feats*3=embed_dims*3/2, H, W)
                # (B, N, num_feats*3=embed_dims*3/2, H, W) --> (B*N, num_feats*3=embed_dims*3/2, H, W)
                # --> (B*N, embed_dims, H, W) --> (B, N, embed_dims, H, W)
                sin_embed = self.adapt_pos3d(sin_embed.flatten(0, 1)).view(x.size())
                pos_embed = pos_embed + sin_embed   # (B, N, embed_dims, H, W)
            else:
                pos_embeds = []
        ...
        # 3D anchor points先生成位置编码，然后利用query_embedding生成初始的object queries PE.
        ...
        outs_dec, _ = self.transformer(x,   # (B, N, embed_dim, H, W)
                                       masks,   # (B, N, H, W)
    ...
    def _get_target_single(self,
        ...
        Args:
            cls_score (Tensor): Box score logits from a single decoder layer
                for one image. Shape [num_query, cls_out_channels].                 # (N_query, n_cls)
            bbox_pred (Tensor): Sigmoid outputs from a single decoder layer
                for one image, with normalized coordinate (cx, cy, w, h) and
                shape [num_query, 4].                                               # (N_query, code_size)
            gt_bboxes (Tensor): Ground truth bboxes for one image with
                shape (num_gts, 4) in [tl_x, tl_y, br_x, br_y] format.              # (N_gt, 9)
            gt_labels (Tensor): Ground truth class indices for one image
                with shape (num_gts, ).                                             # (N_gt, )
    ...
    def loss_single(self,
        ...
        Args:
            cls_scores (Tensor): Box score logits from a single decoder layer
                for all images. Shape [bs, num_query, cls_out_channels].        # (B, N_query, n_cls)
            bbox_preds (Tensor): Sigmoid outputs from a single decoder layer
                for all images, with normalized coordinate (cx, cy, w, h) and
                shape [bs, num_query, 4].                                       # (B, N_query, code_size)
            gt_bboxes_list (list[Tensor]): Ground truth bboxes for each image
                with shape (num_gts, 4) in [tl_x, tl_y, br_x, br_y] format.     # List[(N_gt0, 9), (N_gt1, 9), ...]
            gt_labels_list (list[Tensor]): Ground truth class indices for each
                image with shape (num_gts, ).       # List[(N_gt0, ), (N_gt1, ), ...]
            gt_bboxes_ignore_list (list[Tensor], optional): Bounding
                boxes which can be ignored for each image. Default None.
        Returns:
        ...
        cls_reg_targets = self.get_targets(cls_scores_list,     # List[(N_query, n_cls), (N_query, n_cls), ...]
                                           bbox_preds_list,     # List[(N_query, code_size), (N_query, code_size), ...]
                                           gt_bboxes_list,      # List[(N_gt0, 9), (N_gt1, 9), ...]
                                           gt_labels_list,      # List[(N_gt0, ), (N_gt1, ), ...]
                                           gt_bboxes_ignore_list)
        # cls_reg_targets:
        # labels_list: [(num_query, ),  (num_query, ), ...]     len=batch_size
        # label_weights_list:  [(num_query, ),  (num_query, ), ...]     len=batch_size
        # bbox_targets_list: [(num_query, 7/9),  (num_query, 7/9), ...]     len=batch_size    7: (cx, cy, cz, dx, dy, dz, ry, vx, vy)
        # bbox_weights_list: [(num_query, 7/9),  (num_query, 7/9), ...]     len=batch_size
        ...
        bbox_targets = torch.cat(bbox_targets_list, 0)      # (B*num_query, 7/9)
        bbox_weights = torch.cat(bbox_weights_list, 0)      # (B*num_query, 7/9)

        # classification loss
        cls_scores = cls_scores.reshape(-1, self.cls_out_channels)      # (B*num_query, n_cls)
        ...
        # regression L1 loss
        # (B*N_query, code_size)    code_size: (cx, cy, log(dx), log(dy), cz, log(dz), sin(ry), cos(ry), vx, vy)
        bbox_preds = bbox_preds.reshape(-1, bbox_preds.size(-1))
        # normalized_bbox_targets: (B*num_query, code_size)
        # code_size: (cx, cy, cz, dx, dy, dz, ry, vx, vy) --> (cx, cy, log(dx), log(dy), cz, log(dz), sin(ry), cos(ry), vx, vy)
    ...
    def get_bboxes(self, preds_dicts, img_metas, rescale=False):
        ...
        # preds_dicts: List[predictions_dict0, predictions_dict1, ...]
        # predictions_dict = {
        ...
        for i in range(num_samples):
            ...
            # }
            bboxes = preds['bboxes']
            # 预测的bboxes是gravity_center形式的, 这里再将其转化为bottom_center形式.
