...
class PETRHitHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is PETRHitHead):
        ...
        super(PETRHitHead, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is PETRHitHead:
            convert_dict = {
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        x = mlvl_feats[0]   # (B, N_view, C=256, H, W)  只选择一个level的图像特征.
        batch_size, num_cams, C, fh, fw = x.size()
        ...
        outs_dec, _ = self.transformer(x,   # (B, N_view, embed_dim, H, W)
                                       ...
                                       reference_points=reference_points_realmetric,  # (B, N_query, 3)
                                       lidar2img=lidar2img,  # (B, N_view, 4, 4)
                                       reg_branch=self.reg_branches,    # 没有进行box_refine, 因此没有用到reg_branches.
    ...
    def loss_single(self,
        ...
        Args:
            ...
            bbox_preds (Tensor): Sigmoid outputs from a single decoder layer
                ...
                shape [bs, num_query, code_size].                           # (B, N_query, code_size)
            gt_bboxes_list (list[Tensor]): Ground truth bboxes for each image
    ...
    def get_bboxes(self, preds_dicts, img_metas, rescale=False):
        ...
        Args:
            preds_dicts (dcit): Prediction results.
            {
