...
class PadMultiViewImage(object):
    ...
    def _pad_img(self, results):
        ...
        results['img'] = padded_img     # List[(H, W, 3), (H, W, 3), ...]
...
class ResizeCropFlipImage(object):
    ...
    def __call__(self, results):
        ...
        new_imgs = []
        # 每个view都使用相同的aug.
        # resize的比例, resize_dims后图像的size, crop参数, bool, 旋转角度.
        ...
        for i in range(N):
            ...
            img, ida_mat = self._img_transform(
                img,
                resize=resize,  # float
                resize_dims=resize_dims,  # (resize_W, resize_H)
                crop=crop,  # (crop_w, crop_h, crop_w + fW, crop_h + fH)
                flip=flip,  # bool
                rotate=rotate,  # float
            )
            new_imgs.append(np.array(img).astype(np.float32))   # (H, W, 3)
            # 进行了图像增广后, 对相机内参也根据增广矩阵进行更新，这样3D->2D能够保持一致
            results['intrinsics'][i][:3, :3] = ida_mat @ results['intrinsics'][i][:3, :3]

        results['multi_view_resize'] = resize       # float
        results['multi_view_resize_dims'] = resize_dims     # (2, )   (resize_H, resize_W)
        results['multi_view_crop'] = crop       # (4, )     (crop_w, crop_h, crop_w + fW, crop_h + fH)
        results['multi_view_flip'] = flip       # bool
        results['multi_view_rotate'] = rotate   # float

        results["img"] = new_imgs   # List[(H, W, 3), (H, W, 3), ...]
        # 由于内参进行了更新, 也对lidar2img进行更新.   # List[(4, 4), (4, 4), ...]
        results['lidar2img'] = [results['intrinsics'][i] @ results['extrinsics'][i] for i in range(len(results['extrinsics']))]
...
@PIPELINES.register_module()
class ResizeCropFlipImageV2(ResizeCropFlipImage):
    """Random resize, Crop and flip the image
    Args:
        size (tuple, optional): Fixed padding size.
    """
    ...
    def __init__(self, data_aug_conf=None, training=True):
        super(ResizeCropFlipImageV2, self).__init__(data_aug_conf=data_aug_conf, training=training)
    ...
    def _sample_augmentation(self):
        ...
        if self.training:
            ...
            resize += np.random.uniform(*self.data_aug_conf['resize'])  # resize的比例, 位于[Win/W − 0.06, Win/W + 0.11]之间.
            resize_dims = (int(W * resize), int(H * resize))  # resize后的size
            newW, newH = resize_dims
            # crop
            crop_h = int((1 - np.random.uniform(*self.data_aug_conf['crop_h'])) * newH) - fH  # s * H - H_in
            crop_w = int(np.random.uniform(0, max(0, newW - fW)))  # max(0, s * W - W_in)
            crop = (crop_w, crop_h, crop_w + fW, crop_h + fH)
            # flip
            flip = self.data_aug_conf['flip'] and np.random.choice([0, 1])
            # rotate
            rotate = np.random.uniform(*self.data_aug_conf['rot'])

        else:
            resize = float(fW) / float(W)
            resize += self.data_aug_conf.get('resize_test', 0.0)
            resize_dims = (int(W * resize), int(H * resize))
            newW, newH = resize_dims
            crop_h = int((1 - np.mean(self.data_aug_conf['crop_h'])) * newH) - fH
            crop_w = int(max(0, newW - fW) / 2)
            crop = (crop_w, crop_h, crop_w + fW, crop_h + fH)
            flip = False
            rotate = 0
...
class GlobalRotScaleTransImage(object):
    ...
    def rotate_bev_along_z(self, results, angle):
        ...
        for view in range(num_view):
            # 由于对3D场景进行了旋转，为了保持3D->2D的一致性, 利用rot_mat_inv更新lidar2img，消除旋转的影响.
    ...
    def scale_xyz(self, results, scale_ratio):
        ...
        num_view = len(results["lidar2img"])
        for view in range(num_view):
            # 由于对3D场景进行了scale，为了保持3D->2D的一致性, 利用rot_mat_inv更新lidar2img，消除scale的影响.
            results["lidar2img"][view] = (torch.tensor(results["lidar2img"][view]).float() @ rot_mat_inv).numpy()
            results["extrinsics"][view] = (torch.tensor(results["extrinsics"][view]).float() @ rot_mat_inv).numpy()
